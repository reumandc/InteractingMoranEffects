...
\title{Supporting information for ``How environmental drivers of synchrony interact''}
\date{}
...
patterns known to occur in central and southern CA, and also to have another the same number
of time series per selected region.

Wave nutrient data were also available for each quarter at each location. Wave data were derived 
...
\section{Supporting results for kelp}
