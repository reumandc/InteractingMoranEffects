...
$N=5$, $v_{11}=v{22}=0.6$ and $v_{12}=0.3$.
...
producing oscillations for certain values of the parameters. For instance, for case study C, we use $c_1=0$ and $c_2=-4/9$, values
which cause the expression $\left| \frac{1}{1-c_1 \mu - c_2 \mu^2} \right|^2$ to have a single peak at
frequency $0.25$ cycles per sampling interval (i.e., $1$ cycle per year). It makes sense that the synchrony quantities
...
were used for panels a-c and d-f, respectively. Parameters were:
$N=5$, $c_1=0$, $c_2=-4/9$, $w_{11}=w{22}=0.6$ and $w_{12}=0.3$ for both sets of panels;
$l_1=0$ and $l_2=1$ for panels a-c; and 
$l_1=1$ and $l_2=0$ for panels d-f.
}\label{fig:rhosForC}
\end{figure}
