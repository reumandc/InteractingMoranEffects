...
%*DONE Summary
We provided a general mathematical theory of interactions between Moran effects. When two related spatially 
...
two drivers were unrelated, a new mechanism of population spatial synchrony.
Our general theory illuminates precisely how timings of 
influences of the Moran drivers on populations can interact with relationships between the
drivers to produce interaction effects, which may vary by timescale in both their strength and
sign. Moran effects are very common. Interactions between Moran effects have been tested for, and observed, twice previously, and may
be common [\cite{Sheppard2019,CastoraniSheppardKelp}; see also below]. 
Our study is the first to provide a comprehensive theory of the new mechanism. 
We used our theory and several case studies based on models and 
California kelp populations to provide improved intuition about the mechanism.
...
investigate cross synchrony of environmental variables, and could then use our theory
...
data are available, test directly for interactions. 

%*Revist intuition
We revisit and extend the intuition behind interacting Moran effects, using the central CA kelp
example as a vehicle \citep{CastoraniSheppardKelp}. Large winter waves 
...
annual-timescale synchrony. Sub-annual lags and delays make essentially no difference, 
...
as in Fig. \ref{fig:IntroTheory}, and as captured mathematically in (\ref{rhoww_T1to3}).
...
the two papers have complementary emphases. In spite of numerous methodological choices which differed between the 
...
in central CA, but negative interactions in southern CA at annual timescales and in both central
and southern CA at long timescales. The Fourier approach of this study was designed to facilitate 
mathematical examination of interactions between Moran effect as a general mechanism, whereas the 
...
The approach of this study relates indirectly to the approach of \cite{Anderson2021}, though that
...
spatial extent, than might be expected given the strength or extent of 
synchrony of environmental drivers \citep{Herfindal2022}.
Antagonistic (negative) interaction effects between Moran drivers may be one common and previously
unrecognized mechanism contributing to this discrepancy. On the other hand,
a recent paper descibed an ``enhanced Moran effect'' by which specific 
patterns of temporal autocorrelation
in Moran drivers can theoretically cause greater synchrony in populations 
than in the drivers themselves \citep{Massie2015}.
Synergistic (positive) interaction effects is another mechanism by which populations can
...
%DECIDED NOT TO WRITE*Revisitation of diffs between SoCal and CentCal
