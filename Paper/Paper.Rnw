...
and it will turn out (see below, and \emph{Discussion}) that a timescale-specific 
...
%***DAN: Right now sup mat section S7 is not cited. It only has figures and tables, not sure I plan to 
%add anything to it other than that. Either get rid of it as a section, or else cite it in some of the places 
%where you currently cite the figures and tables in it. My current hunch is that the first of those options is
%better.
...
(called SB; see Fig. \ref{fig:map}). Each kelp measurement is an estimate of mean quarterly kelp biomass per unit 
useable habitat in a 500m stretch of coastline. We used coastline segments where kelp was persistent
...
and nutrients are known to influence kelp population dynamics and synchrony 
...
when waves are large \citep{Dayton1992,Cavanaugh2011,Bell2015}. Thus wave effects 
...
\citep{Zimmerman1986,Cavanaugh2011,Bell2015}. 
...
%***DAN: check this section number, and if possible give a specific formula of theorem or something, and also 
%check the below text and maybe augment it
...
All computations were done in R version 3.4.4 on a laptop running Ubuntu Linux
...
and phase almost $0$ at long timescales (Fig. \ref{fig:TheoryCaseA}b).
...
the size of the $3$q relative lag of this scenario, relative to $\sigma$,
is again captured by the phase of $f_{\mcP^{(1)}} \conj{f_{\mcP^{(2)}}}$ (Fig. \ref{fig:TheoryCaseA}f).
...
%***DAN: Don't forget to appropriately use q in place of quarter from here on as you edit. If you forget, do a 
%search and replace.

%Case study C, new
CaA and CaB assumed white noise processes, and therefore real-valued $\rho_{\epsilon^{(1)} \epsilon^{(2)}}$
and $l_n=0$ (SI sections \ref{SM-sect:examples_cross_sync}).
...
$\epsilon^{(1)}$ effects were lagged and positive, and $\epsilon^{(2)}$ effects were immediate and negative
(Fig. \ref{fig:TheoryCaseA} caption). The lagged positive effects of $\epsilon^{(1)}$ correspond to 
$l_{e1}=1$q, in the language of Fig. \ref{fig:IntroTheory}. But, for that figure, we assumed environmental 
variables had positive effects on populations, whereas for CaC $\epsilon^{(2)}$ had negative effects.
Immediate negative effects of an annually oscillating variable are the same as positive effects lagged 
by $2$q, so we can take $l_{e2}=2$q for CaC.  
%***DAN: pretty confused, come back to this
...
(see above text and Fig. \ref{fig:TheoryCaseA}a,e,i,m),
...
depend strongly on timescale, even having different signs on different timescales.
First, for all of our regions and for essentially all timescales, the magintude of interaction 
...
\ref{fig:SummaryKelpFig}c,f). Finally, interactions depended on timescale, e.g., for the CCA1 
region they were positive on annual timescales (Fig. \ref{fig:SummaryKelpFig}a) and
...
by a term quantifying the autoregressive nature of kelp dynamics,
$1/|f_{\mcB}|^2$. These components are plotted in Fig. \ref{fig:directMoran} (CCA1 and SB regions) and 
Fig. \ref{SM-fig:directMoran_SM_lag4} (CCA2 region) so their 
effects on synchrony can be assessed across timescales and regions. 
Both nitrates and waves fluctuate seasonally in CA (REFS). 
%***DAN: Fugure out a ref for this later
...
Nitrates and waves are also synchronous on long timescales $>8$y in CA 
...
synchrony observed in kelp (Fig. \ref{fig:SummaryKelpFig}c,f, green and blue lines).
Kelp synchrony was generally stronger in the CCA1 region compared to the SB region 
(compare Fig. \ref{fig:SummaryKelpFig}a-c to d-f) in part because the synchrony of nitrates, and of 
waves, was more pronounced in the CCA1 region than in the SB region 
(compare Fig. \ref{fig:directMoran}b,g,l to q,v,aa, and 
Fig. \ref{fig:directMoran}d,i,n to s,x,cc), and also because waves
had a stronger influence on kelp in CCA1 compared to SB (compare 
Fig. \ref{fig:directMoran}c,h,m to r,w,bb).
Regression coefficients which determined $f_{\mcP^{(1)}}$, $f_{\mcP^{(2)}}$
and $f_{\mcB}$ (see Methods) are in Table \ref{SM-tab:regcoefs}.

%Interactions between Moran effects, CCA1
Interactions between Moran effects in CCA1 were parallel to scenario 1 of theoretical case study C.
In CCA1, nitrates have a one-quarter-delayed positive effect on observed 
kelp populations and waves have immediate negative effect (REFS), 
%***DAN: ask Tom/Kyle/Max what is a good reference for this
just as $\epsilon^{(1)}$ effects in case study C, scenario 1, 
were positive and delayed by a quarter and $\epsilon^{(2)}$ effects were negative and immediate.
Delayed effects are reflected, for CCA1, in Fig. \ref{fig:indirectMoran}a, and for 
case study C scenario 1, in Fig. \ref{fig:TheoryCaseA}j, both of which show 
...
the CCA1 entries for $p_0^{(1)}$ were close to $0$ and those for $p_1^{(1)}$ were positive.
Immediate negative effects of waves in CCA1 are reflected in Fig. \ref{fig:indirectMoran}b,
and for case study C scenario 1, in Fig. \ref{fig:TheoryCaseA}j, both of which show 
that $f_{\mcP^{(2)}}$ has phase $-\pi$ at all timescales. See also the CCA1 $p_0^{(2)}$
coefficients in Table \ref{SM-tab:regcoefs}, which are negative.
In CCA1, annual peak nitrates tend to come in spring, whereas annual peak waves 
tend to come in winter (REFS). 
%***DAN: add refs later
Thus annual nitrate peaks tend to lag annual 
wave peaks by one quarter, just as $\epsilon^{(1)}$ peaks lagged $\epsilon^{(2)}$
peaks by a quarter in theoretical case study C, scenario 1.
This is reflected, for CCA1, in Fig. \ref{fig:indirectMoran}d, and for case study C scenario 1 
...
effects of nitrates on kelp tend to come in the summer, and the annual negative effects of waves come in the winter,
producing reinforcing interactions between Moran effects. Likewise, the $\pi/2$ phase 
of $f_{\mcP^{(1)}}\overline{f_{\mcP^{(2)}}}$ (Fig. \ref{fig:indirectMoran}c for CCA1, Fig. \ref{fig:TheoryCaseA}k for case study C scenario 1)
approximately cancels the $-\pi/2$ phase of $\rho_{\epsilon^{(1)}\epsilon^{(2)}}$ 
(Fig. \ref{fig:indirectMoran}d for CCA1, Fig. \ref{fig:TheoryCaseA}k for case study C scenario 1) on annual timescales, so that  
$f_{\mcP^{(1)}}\overline{f_{\mcP^{(2)}}}\rho_{\epsilon^{(1)}\epsilon^{(2)}}$ has phase 
approximately $0$, and the interaction-effect term 
$\Rp(f_{\mcP^{(1)}}\overline{f_{\mcP^{(2)}}}\rho_{\epsilon^{(1)}\epsilon^{(2)}})$ is strongly positive 
on annual timescales in both the theoretical and emprical cases. 
%***DAN: read the below sentence again and see if it sucks
Interactions between Moran effects on long-timescales ($>8$y) were also similar for both CCA1 and
for case study C, scenario 1, and for the same reason in both cases: on long timescales, sub-annual lags
...
Interactions between Moran effects in CCA2 operated in a similar manner to CCA1 (Fig. \ref{SM-fig:indirectMoran_SM_lag4}).
...
%   Tom also told me in a meeting zoom call that in central CA growth of new fronds will be slower because 
%there are not other fronds (left) on the same plant to provide a sugar subsidy to new fronds. In central CA,
%basically all fronds from the same plant are ripped out.

%Need a paragraph (already mentioned above) on how a frequency/timescale-specific approach
%is necessary for all this. Need to think more about it first, i.e., how would you attempt to 
%do this without such an approach, and how far would you get? Or maybe that is not quite the 
%right at it (though you should try thinking that through and see what you get), maybe another
%possibly better way would be to see that everything we list here can depend on timescale.
%You can get a non-timescale-specific approach just by integrating our main equation across
%frequencies. However, that will obscure the fact that you can get strong positive interaction 
%effects on some timescales and strong negative ones on others. What would you get in our 
%theoretical case studies if you integrated and would it reveal what's really going on? What 
%are the implications of timescale structure for practical situations where populations and
%environmental variables are measured over a certain typical duration (i.e, short time series)?
...
given timescale when lags align, i.e, $l_{e1}-l_{e2}+l_n=0$, as in panel (a), so that environmental
...
\begin{figure}
\caption{To fill. See the online version for a color
rendering of all figures.}\label{fig:map}
\end{figure}
...
\caption{Main components of the new theory as applied kelp, central CA 1 (CCA1) region (a-c) and 
...
The CCA1 and SB regional results, respectively, approximately parallel the 
results for theoretical case
study C scenarios 1 and 2 (Fig. \ref{fig:TheoryCaseA}i,m; see text for details).
...
kelp lag $n=4$ (see (\ref{main_model_1}) for the meaning of $n$); 
...
\caption{Components of direct Moran effects for kelp, for the CCA1 (a-o)
and SB regions (p-dd). The index $1$ denotes nitrates, and $2$ denotes waves.
Vertical axis extents are the same for corresponding panels for the
two regions, to facilitate comparisons. 
See Fig. \ref{SM-fig:directMoran_SM_lag4} for the central CCA2 region,
for which results were similar to CCA1. This figure used 
kelp lag $n=4$; see Figs \ref{SM-fig:directMoran_SM_lag8} 
and \ref{SM-fig:directMoran_SM_lag12} for $n=8, 12$, respectively.
See the online version for a color
rendering of all figures.}\label{fig:directMoran}
\end{figure}
...
\caption{Filler. See the online version for a color
