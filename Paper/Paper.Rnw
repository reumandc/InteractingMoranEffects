...
illuminate this newly observed but potentially quite common mechanism of spatiotemporal population dynamics. 
...
The core of this paper is the new theory, but to help illustrate that theory, fulfilling goal G3 of the 
...
(SB; see Fig. \ref{fig:map}). Each kelp measurement is an estimate of mean quarterly kelp biomass per unit 
...
effects appear delayed by about one quarter because our kelp data quantify canopy (surface) biomass, and it can
...
Regression coefficient determined the quantities $b_1,\ldots,b_n$, $p_0^{(1)}$, $p_1^{(1)}$, and $p_0^{(2)}$
in (\ref{main_model_1})-(\ref{main_model_3}), and therefore the quantities $f_{\mcB}$, $f_{\mcP^{(1)}}$ and
...
$\rho_{\epsilon^{(1)}\epsilon^{(2)}}$ in (\ref{rhoww_T1to3}), where $w$ was detrended kelp time series in one of our regions, 
...
All computations were done in R version 3.4.4 on an ordinary laptop which was running Ubuntu Linux
16.04. Complete codes for the entire project workflow are at \url{https://github.com/reumandc/InteractingMoranEffects.git}.

\section{Results}

%Para basically confirming earlier work, as a starter
Helping fulfill goal G2 of the Introduction, our theoretical results 
...
Both nitrates and waves fluctuate seasonally in CA. Thus nitrate and wave 
...
(Fig. \ref{fig:directMoran}l,n,aa,cc), possibly due to the North Pacific Gyre Oscillation [NPGO; \cite{CastoraniSheppardKelp}]. The long-timescale synchrony
