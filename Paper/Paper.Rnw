...
would swing more or less randomly, with small amplitude, and would become asynchronous with each other. 
...
to illuminate the inner workings of the general mechanism we describe rather than being an exploration
of kelp dynamics, specifically. Overall, this study introduces a general theory of Moran interactions and uses it to conceptually
...
See SI section \ref{SM-sect:examples_cross_sync} for detailed examples.
...
%***DAN: Upon further reflection, I'm not totally sure of that. Those examples in the sup mat are pretty technical.
%If I brought a version of them here, it would have to be a stripped down, intuitive, version, with the math removed.
%And the new fig would have to just be panel c or f from Fig. S1. That's still possible to do, but does it really
%help more than just the verbal description above?
...
%***DAN: I am currently (2022 11 17) unsure of how much of this belongs in the main text
%versus the sup mat, and how to structure new material I will add to the sup mat, which will 
%influence what section I site above (currently not the right section). 
%***DAN: One way to go may be to put the 2 paras immediately above at the end of the theory
%derivation section in sup mat, and just state here that interpretations of theory 
%consituents, as well as the derivation itself, are in that section. The you could add 
%a detailed elaboration of the theoretical constituents for the theory case studies to S6,
%including a reinstatement of the components of Fig. 5 that were cut. And then I guess you could
%add a new SI section S7 which would have a detailed exploration of the theory constituents
%for kelp.
%***DAN: Decision, and also started implementing a bit: Keep the above paras, no extension of
%them in sup mat, but do add 
%a detailed elaboration of the theoretical constituents for the theory case studies to S6,
%including a reinstatement of the components of Fig. 5 that were cut. And then 
%add a new SI section S7 which would have a detailed exploration of the theory constituents
%for kelp.
...
%See SI section \ref{SM-sect:breakdown_case_studies} for a detailed technical breakdown of how the terms in 
%our theory capture the complexities described above.
...
language of Fig. \ref{fig:PedagogAlt2}; whereas 
$\epsilon^{(2)}$ effects were unlagged and negative, i.e., $l_{e2}=0$ (Methods). 
We compare CaseC to the Fig. \ref{fig:PedagogAlt2} $\beta$ scenarios instead of the Fig. \ref{fig:PedagogAlt} 
...
Fig. \ref{fig:PedagogAlt2}c, d: the 
...
similar to Fig. \ref{fig:PedagogAlt2}a, b: the annual positive population effects of $\epsilon^{(1)}$
...
Fig. \ref{SM-fig:directMoran_SM_lag4} for CCA2), which produced 
...
(Fig. \ref{fig:noise}g,h,p,q for the CCA1 and SB regions, Fig. \ref{SM-fig:directMoran_SM_lag4} 
...
and $f_{\mcB}$ are in Table \ref{SM-tab:regcoefs}.
%***DAN: Later you will have to make sure all rules are followed for citing all sup mat figures and 
%tables and sections from the main text, in order. I think currently they are not all cited, in part
%because I have been shifting around what information is where in the whole paper. 
...
The delayed effects of nitrates in CCA1 are reflected in Table \ref{SM-tab:regcoefs}, where 
regression results for $p_0^{(1)}$ are close to $0$ and those for $p_1^{(1)}$ are positive.
...
%that $f_{\mcP^{(2)}}$ has phase $-\pi$ at all timescales. 
Immediate negative effects of waves in CCA1 are also reflected in Table \ref{SM-tab:regcoefs},
...
Thus lags compound on the annual timescale, in the CCA1 case as in CaseC1: the annual positive 
...
%of $f_{\mcP^{(1)}}\overline{f_{\mcP^{(2)}}}$ on annual timescales 
%(Fig. \ref{fig:indirectMoran}c for CCA1, Fig. \ref{fig:TheoryCaseA}k for CaseC1)
...
%(Fig. \ref{fig:indirectMoran}a v. p; see also 
see Table \ref{SM-tab:regcoefs}, where $p_0^{(1)}$ terms were close to zero for CCA1 and positive for SB,
whereas $p_1^{(1)}$ terms were positive for CCA1 and close to zero for SB. Wave effects were negative and
immediate in both regions, i.e., $p_0^{(2)}<0$ in Table \ref{SM-tab:regcoefs}. 
...
CCA1 (Fig. \ref{fig:noise}l v. c). 
Whereas in CCA1, the tendency of nitrate annual peaks to follow wave annual peaks by $1$q, together with to the tendency of 
...
for \citep{Sheppard2019,CastoraniSheppardKelp}, interactions may be common. 
...
phenomena such as the North Pacific Gyre Oscillation or El Ni\~{n}o Southern Oscillation (ENSO).
...
%***DAN: Do we still need this if we're cutting? Are we cutting?
...
%***DAN: Could we put this in sup mat if we're cutting? Are we cutting?
Our results about kelp were consistent with those of \cite{CastoraniSheppardKelp}, though
that study uses distinct methods; our results complement the results of \cite{CastoraniSheppardKelp} 
in important ways. In spite of 
...
%***DAN: I guess you could move this to sup mat if we're cutting
...
% %***DAN: Could def put this in sup mat if we're cutting
...
completely removed by winter waves. Effects of elevated nitrates on the growth of kelp stands already reaching
...
% %para is also a good place to stop, maybe better. 
...
\caption{%\doublespacing 
Similar to Fig. \ref{fig:PedagogAlt}, but for slightly altered scenarios. For Fig. \ref{fig:PedagogAlt},
...
positive population influences (as in the $\alpha$ scenarios of Fig. \ref{fig:PedagogAlt}), 
but that larger values of $\epsilon_i^{(2)}$ have more negative population influences (contrasting
with the Fig. \ref{fig:PedagogAlt} scenarios). In scenario $\beta 1$, 
...
\caption{%\doublespacing 
Theoretical case studies A (panels a-b), B (c-d), C1 (e-h) and C2 (g-h). 
...
%$c_1=0$ and $c_2=-4/9$ 
for case study C. For C1, peaks in the periodic noise process $\epsilon^{(2)}$
