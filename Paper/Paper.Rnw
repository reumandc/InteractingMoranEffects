...
Thus $l_{e1}=1$q, $l_{e2}=0$q, $l_n=1$q, and so the environmental effect alignment measures is 
...
Mechanisms include demographic stochasticity, measurement error, and influences of unmeasured environmental variables. 
Antagonistic interactions between Moran drivers may be a common and previously
unrecognized mechanism also contributing to this discrepancy. On the other hand,
...
Synergistic interaction effects are another mechanism by which populations can
