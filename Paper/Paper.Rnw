...
The core of this paper is the new theory, but to help illustrate that theory, we also apply
it to an exceptional dataset on giant kelp (\emph{Macrocyctis pyrifera}) dynamics off the CA coast, and 
...
%2) Para about the linear modelling - this is how certain parts of the main theoretical equation were estimated,
%about the lags we used, and justification for them etc. Might also need to cite a sup mat section. Maybe 
%we need one single sup mat section called "supplementary methods" or "details of methods" or something. Actually
%let's have two of these, one for data, one for stats
%3) Para about spectral estimates. Brief, cites appropriate parts of Brillinger, uses sup mat if needed. This is how certain
%other parts of the main theoretical equation were estimated. Basically it's just about how to do spectral
%estimates of spectra and cross spectra, and then rest follows from the main theoretical equation.
%4) Somehow need to get across what was done - different parts of main equation were examined to explore the different
%aspects of how the mechanism works for the focal taxon. Elaborate as appropriate.

Data for the project are archived at X.
%***DAN: Need to get the data location from Jon, I think he posted it, right? Maybe he did not put it all i
%one place. Check with him. At the end of the Max/Lawrence paper, several data locations are listed.
%I have an email out to Jon and Max.
All computations were done in R version 3.4.4 on an ordinary laptop which was running Ubuntu Linux
16.04. Complete codes and data are at \url{https://github.com/reumandc/InteractingMoranEffects.git}.
