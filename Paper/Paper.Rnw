...
kelp populations and waves had immediate negative effect (REFS), 
...
%Maybe a para somewwhere about how it can work that synchrony explained by one factor (or the sum of 
%synchrony explained by two) can add up to more than 100% and then interaction effects can bring the total
%down. Basically this means if the two factors were independent, synchrony would be much greater. E.g.,
%CCA1 on long timescales. Might want to use the analogy with two parents pushing a kid on a swing,
%and then multiple sets of such setups. Or maybe make it more official sounding by talking about 
%oscillators instead of swings. Not sure a swing is a good example because it has a very specific 
%intrinsic dynamics. Likewise for a oscillator. Think about it, not sure what to do here. 
...
%better and how it actually works. It may be sufficient to keep this a fairly brief para. One diff is that
%my Fourier approach facilitates analytic examination of interaction effects for the model I consider.
%Their approach is better for dealing with non-stationarity and other features of some datasets. My
%approach facilitates understanding the inner workings of the mechanism of interacting Moran effects, 
%their approach facilitates identifying such effects in datasets.
...
%and ongoing work. Note that you cannot really make dispersal and Moran effects independent/uncorrelated
%in the same way you can make two Moran effects independent/uncorrelated.
