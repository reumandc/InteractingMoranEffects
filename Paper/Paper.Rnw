...
in a wide variety of species ranging from viruses to plants to mammals, and at 
...
is important for system stability \citep{Ghosh2020b,Ghosh2020c,Walter2021}. Our ability to infer the causes of 
...
%Intro to Moran effects and interactions
...
a quantity which depends not only on the standard environmental synchrony 
...
use a simple analogy from everyday experience to begin to provide intuition. 
Consider $N$ children, each riding on their own playground swing and each
being pushed both by their own mother and father. If the fathers from seperate families
were to synchronize their pushes, they would, of course, act as a Moran-like influence, tending to synchronize
...
of the fathers were appropriately coordinated with the pushes of the mothers, the children would swing higher, and
...
would swing more or less randomly and would become asynchronous with each other, demonstrating
an antagonistic interaction between synchronoizing effects, i.e., a tendency for the two synchronizing
effects to cancel. As oscillators, children on swings are distinct in many ways from ecological
populations, not least because swings oscillate on one frequency/on one timescale, whereas populations
typically oscillate simultaneously on many timescales, having a broadband spectrum. 
We will see that a key insight of our theory is the use of a timescale-specific approach: 
environmental signals are decomposed, using Fourier analysis, into components that occur 
on different timescales,
and the nature of interacting Moran effects can be understood on a timescale-by-timescale basis.
...
spatially extended systems. Interaction effects can also be strong. For these reasons and others, development of a 
...
for kelp ecology, we instead use a simplified subset of the available kelp data. Our kelp analysis is intended 
to illuminate the inner workings of the general mechanism we describe rather than being a complete exploration
of kelp dynamics. Overall, this study introduces a general theory of Moran interactions and uses it to conceptually
...
that we began to develop with the swing example in the Introduction. We again focus on a single
...
different assumption. If, as in Fig. \ref{fig:PedagogAlt}a, b, $l_n+l_{e1}=l_{e2}$, then the 
...
Intermediate scenarios are possible, as will be revealed by the theory. If we replace the assumption that
...
antagonistic Moran interactions are reversed, but with the same general concepts nevertheless still operating 
(Fig. \ref{fig:PedagogAlt_SM}). 
...
represented on Figs \ref{fig:PedagogAlt} and \ref{fig:PedagogAlt_SM} by $l_{e1}$ and $l_{e2}$,
relative to the timescale $\sigma$. The lag 
$l_n$ on Figs \ref{fig:PedagogAlt} and \ref{fig:PedagogAlt_SM}  manifests in the theory 
...
(see above text and Figs \ref{fig:PedagogAlt}, \ref{fig:PedagogAlt_SM}) corresponds to the phase of the expression 
...
The derivation of the theory and details supporting the above interpretations of 
constituents of the theory are in SI section \ref{SM-sect:derivation}. 
%***DAN: I need to add some stuff to the SI section mentioned to support the claim that 
%"details supporting the above assertions are in SI section X"
...
lagged by one model time step (1q), i.e.,
...
but now lagged by $3$q, so that $p_3^{(1)}>0$ while $p_i^{(1)}=0$ for $i \neq 3$.
For both CaseA and CaseB, the effects of $\epsilon^{(2)}$
are assumed to be un-lagged and positive, i.e., $p_0^{(2)} > 0$ and $p_i^{(2)}=0$ for $i>0$.
For case study C (CaseC), the effects of $\epsilon^{(1)}$ are positive and lagged by $1$q and the 
effects of $\epsilon^{(2)}$ are unlagged and negative, i.e.,
$p_{1}^{(1)} > 0$ while $p_i^{(1)}=0$ for $i \neq 1$, and 
$p_{0}^{(2)} < 0$ while $p_i^{(2)}=0$ for $i \neq n_2$.
The noise process
...
taken to be a white noise process (see SI section \ref{SM-sect:examples_cross_sync} for details).
...
%***DAN: I comented the below because I want to put it in sup mat. Once you write that section, consider
%referring to it here with something like: "See SI section X for a detailed breakdown of how the terms in 
%our theory capture these complexities."
...
%interaction term in (\ref{rhoww_T1to3}), as described in General Theory. 
Comparing
...
a white noise process for CaseB, $l_n=0$.
...
%***DAN: May again want to cite a new section of sup mat, in a similar way suggested in a comment in the previous paragraph
%Theory captures these complexities because 
...
But CaseC illustrates what can happen when noise processes have periodic components and lagged associations, e.g., 
...
Recall that, for CaseC, $\epsilon^{(1)}$ effects on populations were positive and lagged by $1$q, i.e., $l_{e1}=1$ in the 
language of Fig. \ref{fig:PedagogAlt_SM}; whereas 
$\epsilon^{(2)}$ effects were unlagged and negative, i.e., $l_{e2}=0$ (Methods). 
We compare CaseC to Fig. \ref{fig:PedagogAlt_SM} instead of Fig. \ref{fig:PedagogAlt} because
$\epsilon^{(2)}$ effects were negative for CaseC, the situation considered by Fig. \ref{fig:PedagogAlt_SM}.
For CaseC1, peaks in the periodic
noise process $\epsilon^{(1)}$ were set up to lag peaks in the periodic process $\epsilon^{(2)}$ by
$1$q (Methods, SI section \ref{SM-sect:examples_cross_sync}), so that $l_n=1$. 
Thus $l_n+l_{e1}-l_{e2}=2 \text{q}=0.5 \text{y}$, and, on annual timescales, lags compounded, similar to 
Fig. \ref{fig:PedagogAlt_SM}b: the 
...
$1$q, so that $l_n=-1$. Thus $l_n+l_{e1}-l_{e2}=0$ and, on annual timescales, lags cancelled,
similar to Fig. \ref{fig:PedagogAlt_SM}a: the annual positive population effects of $\epsilon^{(1)}$
...
%***DAN: Maybe cite a new sup mat section here, once written
...
%***DAN: start from here after making the new version of the figure sketched in a separate planning doc
...
%***DAN: You need to combine the current figs 2 and 3, saving that for later.

%***DAN: Stopped here 2022 11 15, 230pm
...
\caption{%\doublespacing 
Theoretical case studies A (panels a-b), B (panels c-d) and C (e-h, two alternative sets of 
parameters used for e-f and g-h, respectively). Left panels (a, c, e, g) show the terms on the right side
...
in panels e-f, and $\epsilon^{(1)}$ lagged $\epsilon^{(2)}$ by the same
amount in g-h.
...
Synch. contrib. = Synchrony contribution; Rel. synch. contrib. = Relative synchrony contribution. 
See the online version for color
...
See Figs \ref{SM-fig:directMoran_SM_lag4} and \ref{SM-fig:indirectMoran_SM_lag4} for CCA2,
for which results were similar to CCA1. This figure used 
kelp lag $n=4$; see Figs \ref{SM-fig:directMoran_SM_lag8}, \ref{SM-fig:indirectMoran_SM_lag8}, 
\ref{SM-fig:directMoran_SM_lag12} and \ref{SM-fig:indirectMoran_SM_lag12} for $n=8$ and$ 12$.
See the online version for color
