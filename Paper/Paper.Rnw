...
\title{Environmental drivers of synchrony and their interactions}
\date{}
...
a different variable in another location. Fig. \ref{fig:IntroTheory} illustrates how population synchrony can 
...
%***DAN: If we decide to go longer, we could add some brief commentary here about how common it should be for 
%spatially extended population systems to be influenced by more than one env var. Maybe also mention these
%env vars may be different vars measured at the same time of year, or the same or different vars measured
%at different times of year, i.e., separated phenologically. Keep it brief, it's just intended to make them 
%this is is going to be important and common, and to make them sorta wonder why this has not been examined already.
%Actually that second thing is done in the Discussion, either don't do it here or else if you do, then take it
%out of there.

%***DAN: Another potentially good Discussion para: make connections to the work (or Wang and Loreau and
%others) that is fusing spatial and community ecology. What are the implications of this work for that work?
%Making such additions may help it appeal to a broader audience, which is something we need to work on as 
%much as we can for Ecol Lett.

%***DAN: The following para is a lot to ask the reader to absorb in the Intro, way too much for 
%an ecological audience. Move it Theory. Can I replace it with something more intuitive, like the
%swing example? Or is that a bad combination of too involved and too informal/non-ecological? 
%If I were to use the swing example I'd have to do it in limited space, to prevent it from 
%spiraling out of control as an overly extended metaphor.
%***DAN: You also might want to add a para to the Intro on why interactions between Moran effects
%should be important, to take up some space upon moving the below para to Theory (and it would also 
%help motivate).
Having demonstrated that interactions between Moran effects can augment or reduce population synchrony, we now 
...
across timescales afterwards if desired. Fig. \ref{fig:PedagogAlt}a,b shows the period-20 Fourier components of two hypothetical,
...
we assume for simplicity that larger values of both environmental variables are beneficial to populations, though the general
theory described below does not require that assumption. If, as in Fig. \ref{fig:PedagogAlt}a, b, $l_n+l_{e1}=l_{e2}$, then the 
...
shows the opposite scenario, for which $l_n+l_{e1}=l_{e2}+\sigma/2$, where $\sigma=20$ is the period. Thus
...
for kelp ecology, we instead use a simplified subset of the available kelp data. Our analysis is intended 
to illuminate the inner workings of the general mechanism we describe rather than being a complete exploration
of kelp dynamics. In the Discussion, we 
extend the arguments of \cite{Sheppard2019} and \cite{CastoraniSheppardKelp} that interactions between Moran 
effects are likely to be common, and hence that our theory seems likely to pertain to numerous 
...
which has $ij^{\text{th}}$ entry $S_{w_i w_j}(f)$. Spectral methods are standard  
...
%***DAN: If we go longer, you could bring these examples here and it would probably be a positive addition of
%about one para and one new Fig. Would be a positive addition because it helps people understand our new metric.

%***DAN: If you go longer, you might think about adding a para to Discussion about how general theories can bind together
%multiple effects that are often understood idiosyncratically or as special cases by ecologists. Not sure this could be done 
%effectively, but I'm always reading papers testing specific things and understanding phenomena in ways like "A can happen
%and B can also happen". For instance, X (which is a good paper) says "We show that environmental fluctuations can reduce 
%synchrony between species and increase stability, yet they can also decouple asynchrony between species and increase 
%population and metapopulation variability." (https://esajournals-onlinelibrary-wiley-com.www2.lib.ku.edu/doi/pdfdirect/10.1002/ecm.1484) 
%Sounds like they are saying both things can occur, we want a general theory
%to understand when each outcome happens. Likewise could cite the old debate on red noise and effects on exitinction risk. 
%There were papers show conflicting results until someone looked for the general theory and then it was resolved. Ecologists
%have a natural proclivity for specific examples, and like results like "A can happen" but as scientists we ought to be 
%seeking general theories. Not sure how to say all this in a constructive and useful as opposed to an annoying way. 
%The assumptions that your dynamical models have an equilibrium and then an approach that assumes weak noise and linearizes 
%about that equilibrium can provide a general theory (as we did here, but also cite several papers of myself and others using 
%this appraoch). Whereas the theory is not completely general, because it does not cover strong noise and strong nonlinearities,
%which do happen in ecology, we consider it better to produce a a general theory based on linearization and then examine
%departures from it under strong noise than it is to use a series of scattershot simulation approaches, which has more than 
%led to conflicting results in the literature which then had to be resolved. In our opinion, generality should be pursued in
%theoretical ecological contexts rather sooner than it is our impression that it is typically pursued, wrking on it 
%early on instead of only after several case-study-type studies based on simulations. The weak-noise linearization approach
%is widely applicable and often more fruitful than simulations (REFS).

%***DAN: Potential reviewers or editors:
%Maybe Holt as editor if we go to Ecography
%Gonzalez
%Loreau?
%Bjornstad
%Holt's postdoc, name slipping my mind
%Barfield? 
%Holt as reviewer? Will he even do it?
%Cohen? Probably a conflict.
%Arnoldi? Bart Haegeman?
%Montoya?

%***DAN: Question for later: Suppose we considered a model with three measured env drivers, would you get
%three-way interactions? You might add the question to the Discussion para about adding dispersal to this model.
%Not sure actually doing that is a good use of my time, though.
...
The phases of these quantities encode the lags in the influences of 
$\epsilon^{(1)}$ and $\epsilon^{(2)}$, 
represented on Fig. \ref{fig:PedagogAlt} by $l_{e1}$ and $l_{e2}$,
relative to the timescale $\sigma$. The lag 
$l_n$ on Fig. \ref{fig:PedagogAlt} manifests in the theory 
through the phase of $\rho_{\epsilon^{(1)}\epsilon^{(2)}}$. 
The expression $l_{e1}-l_{e2}+l_{n}$, which quantifies the degree of mismatch among the lags
(see Introduction and Fig. \ref{fig:PedagogAlt}) corresponds to the phase of the expression 
...
$l_{e1}-l_{e2}+l_n$ is close to zero (Fig. \ref{fig:PedagogAlt}a, b), and is increasingly negative as 
...
(Fig. \ref{fig:PedagogAlt}c, d). The factor $1/|f_{\mcB}|^2$ is how the intrinsic nature of population
dynamics modulates Moran influences.
The derivation of the theory is in SI section \ref{SM-sect:derivation}.
%***DAN: Some of the content of the above para is quite technical, and is more about interpreting the
%inner working of the theory as opposed to just comparing its predictions to the intution from
%Figs 2 and 5. So maybe some of this should be move to a sup mat where I would really
%delve into how the inner workings of the theory correspond to all these lags and alignments
%of lags and so on.

\section{Methods}

%***DAN: Consider renaming our three scenarios as follows: A->A1, B->A2, C1->B1, C2->B2. After all,
%A and B are quite related, so it makes sense to consider the as scenarios within a case study.
%Then it would be more symmetric, and the four rows of panels of Fig. 4 would each correspond to
%a scenario within a case study. 
...
For case study B (henceforth CaseB), which generalizes A, $\epsilon^{(1)}$ is again assumed to have a simple 
...
For case study C (CaseC), the effects of $\epsilon^{(1)}$ are lagged by $n_1$ and the 
...
either lead or lag peaks in $\epsilon^{(2)}$ by $1$q (we consider two scenarios, CaseC1 and CaseC2).
Such scenarios could be realized by annually 
...
Details of the noise processes are in SI section \ref{SM-sect:examples_cross_sync}.
...
The core of this paper is the theory, but to help illustrate that theory, for goal G3 of the 
...
\cite{Walter2021}, and data details are given in those papers and in SI section \ref{SM-sect:data};
...
%***DAN: Probably should shift this definition of what w stands for to the previous paragraph, since I was 
%already using w in this context, there.
...
%***DAN: likewise these ought to have been defined in the previous paragraph, I think
...
long timescales (Fig. \ref{fig:TheoryCaseA}i); and CaseC2 showed negative interactions on all timescales 
...
for all case studies (Fig. \ref{fig:TheoryCaseA}a,e,i,m), and even differed in sign across timescales for CaseA, CaseB and CaseC1. 
...
the two noise variables reinforce each other. 
%***DAN: Upon re-reading aftre taking several months break from this project, I find myself skimming
%the following sentences which are supposed to be showing how the theory captures what is going on.
%It makes me wonder if explaining how theory corresponds to intuition is maybe beyond the audience,
%and that part should be consistent relegated to sup mat, and instead in the main text we just 
%present the results of theory, and explain how they correspond to the intuition developed in Fig. 2.
%I think that would also allow us to put Fig. 4 panels b,c,f,g,j,k,n,o into sup mat, massively
%simplifying the presentation. Keep the last sentence of this para here, though, along with panels 
%d,h,l,p.
...
which has phase $-\pi$ at short timescales
...
Fig. \ref{fig:TheoryCaseA}a and d shows the additional influence of autoregressive population effects, which
...
%***DAN: Note that case B was introduced in Methods as having a lag of length n in the effects
%of epsilon 1, but we immediate specify n=3 in Results. Do we want to instead intro it as n=3
%from the beginning?
CaseB reveals how Moran effects can interact when lags are longer than 1 model time step. 
For CaseB, $\epsilon^{(1)}$ effects on populations were lagged by $n$q but 
...
a white noise process for CaseB, $\rho_{\epsilon^{(1)} \epsilon^{(2)}}$ was a real number and $l_n=0$.
...
(Fig. \ref{fig:TheoryCaseA}e). 
%***DAN: We could again put this "theory captures these complexities" business in sup mat, where
%we could anyway explain in greater detail. Also put the middle panels there.
Theory captures these complexities because 
the size of the $3$q sum of lags for this scenario, relative to $\sigma$,
is again captured by the phase of $f_{\mcP^{(1)}} \conj{f_{\mcP^{(2)}}}$; the phase of that
quantity, represented by colors on Fig. \ref{fig:TheoryCaseA}f,g, fluctuates analogously to the
interaction effects on Fig. \ref{fig:TheoryCaseA}a.

%Case study C, new
CaseA and CaseB assumed white noise processes, and therefore 
real-valued $\rho_{\epsilon^{(1)} \epsilon^{(2)}}$ and $l_n=0$.
But CaseC illustrates what can happen when noise processes have periodic components and lagged interdependences, e.g., 
...
For CaseC, $\epsilon^{(1)}$ effects on populations were lagged by $n_1$q and 
$\epsilon^{(2)}$ effects were lagged by $n_2$q (Methods). 
%***DAN: give the exact lag values used in methods. Also the coefficient values, probably. 
We used $n_1=1$, $n_2=0$, $p_1^{(1)}=3$ and $p_0^{(2)}=-1.5$, so 
$\epsilon^{(1)}$ effects were lagged and positive, and $\epsilon^{(2)}$ effects 
were immediate and negative. The language and logic of 
Fig. \ref{fig:PedagogAlt} does not directly apply, because we there assumed, in order to introduce concepts simply,
that environmental effects were positive, and we now take $p_0^{(2)}=-1.5$. But similar logic does apply 
(Fig. \ref{fig:PedagogAlt_SM}), with
both the new and previous paradigms covered by our general theory.
%***DAN: It is asking way too much of the reader to spend the labor to go through Fig. 5 and update their
%understanding to become more general than the original Fig. 2 just because of this one sentence. I am
%asking them to spend maybe 15-20 minutes puzzling over Fig. 5 based on this one sentence of main text.
%People won't do that, instead they'll just accept a more surface-level understanding and blunder onward.
%Suppose I moved the text referring to Fig. 2 from the Intro to the Theory, and then also cited what is
%currently Fig. 5 there (it would become Fig. 3). So the reader would be prepared for both positive and
%negative effects, and when they got here they would be ready to absorb this example. I think this is a 
%smart idea.
For CaseC1 (Fig. \ref{fig:TheoryCaseA}i-l), peaks in the periodic
noise process $\epsilon^{(1)}$ lagged peaks in the periodic process $\epsilon^{(2)}$ by
$1$q (Methods, SI section \ref{SM-sect:examples_cross_sync}). Thus lags compounded: the 
...
Contrastingly, for CaseC2 (Fig. \ref{fig:TheoryCaseA}m-p), peaks in the periodic
...
%***DAN: More "theory captures these complexities" stuff that should be sent to sup mat.
%No one will absorb this kind of stuff who isn't incredibly dedicated, and therefore
%will be willing to read sup mat. Make the main text more about comparing the intution 
%from fig 2 and 5 with the predictions of theory, while black-boxing how the theory
%actually works.
...
$\rho_{\epsilon^{(1)} \epsilon^{(2)}}$ captures the degree of alignment between 
...
reflected in the real part of this quantity, which is half the interaction term in our theory (\ref{rhoww_T1to3}).
...
%***DAN: Do I really want to spend all this space (and reader mental energy), including 
%a very complex figure, on direct Moran effects, when the paper is not even about that?
%***DAN: And this paragraph also requires plotting all these inner portions of the
%theory, which I am starting to think is not really going to work for an ecological 
%audience.
The way direct Moran effects work in our theory is fairly straightforward. 
%***DAN: Why is the following sentence not in the Theory section?
For instance, the direct Moran effect
term in (\ref{rhoww_T1to3}) for nitrates, 
$|f_{\mcP^{(1)}}|^2 \rho_{\epsilon^{(1)} \epsilon^{(1)}}/|f_{\mcB}|^2$, 
equals the synchrony of the nitrate time series themselves, 
...
\citep{CastoraniSheppardKelp}. The long-timescale synchrony
...
%***DAN: In previous comments I have begun outlining how I might simplify
%and make the paper more about comparing the intuition of Fig. 2 and 5 to 
%the *predictions* of theory, rather than taking the reader through all the
%inner workings of theory. I like those thoughts so far, but they present some chanllenges
%when we come to this paragraph. Much of what is stated below can be demonstrated
%by looking at the estimates of the constituents of the theory, but I think that'll
%be too confusing. So we have to come up with other ways to demo it. I think the 
%solution is: we feel free to plot our synchrony and cross synchrony measures,
%rho_epsiloni_epsilonj, even though in the case of i differnt from j, we have to
%plot a complex quantity. But instead of plotting the fs, we just talk about
%lags garnered from regression.
...
just as $\epsilon^{(1)}$ effects in CaseC 
were positive and delayed by $1$q and $\epsilon^{(2)}$ effects were negative and immediate.
%***DAN: So we would drop the following sentence, or send it to a new section in 
%sup mat where we systematically explored the constituents of the theory. Instead
%we would just rely on the regression results which follow.
Delayed effects are reflected, for CCA1, in Fig. \ref{fig:indirectMoran}a, and for 
CaseC1, in Fig. \ref{fig:TheoryCaseA}j, both of which show 
that $f_{\mcP^{(1)}}$ has phase about $-\pi/2$ at the annual timescale. 
%***DAN: The above is pretty horrible. The reader has to locate two panels on two differently
%formatted and very complex figures and compare them to see this! They will not do that!
The delay is also 
...
%***DAN: On the other hand, the contents of the table are pretty clear and intutitive to
%an ecological audience, and can be used to match the intution of lags (Figs 2 and 5) with the
%*outputs* of the theory, without plotting all the inner working in the main text. Even just
%plotting both phase and magnitude on the same axes is going to be a stretch, and we have been
%doing that a LOT! I'm really thinking that dragging the reader through all the inner workings
%of this theory is just not going to work, unless I want to publish in Theoretical Population
%Biology or something.
%***DAN: So we would also drop the following sentence, relying instead
%on the regression results which follow.
Immediate negative effects of waves in CCA1 are reflected in Fig. \ref{fig:indirectMoran}b,g,l
and for CaseC1 in Fig. \ref{fig:TheoryCaseA}j, both of which show 
...
%***DAN: We would keep the portion of the following sentence that is about CCA1,
%and we would keep a version of Fig 8d, which shows the cross synchrony between 
%these drivers corresponds to a lag of 1 quarter. The reference to 4k will just 
%be dropped, since anyway we built that theoretical case study to have that lag 
%so don't need a confusing spectral reflection of the lag. In the sup mat section 
%where we go through the constituents of the theory in detail can have this kinda 
%stuff.
This is reflected, for CCA1, in Fig. \ref{fig:indirectMoran}d, and for CaseC1 
...
%***DAN: Following sentence moved to sup mat.
Likewise, the $\pi/2$ phase 
of $f_{\mcP^{(1)}}\overline{f_{\mcP^{(2)}}}$ on annual timescales (Fig. \ref{fig:indirectMoran}c for CCA1, Fig. \ref{fig:TheoryCaseA}k for CaseC1)
approximately cancels the $-\pi/2$ phase of $\rho_{\epsilon^{(1)}\epsilon^{(2)}}$ on annual timescales
(Fig. \ref{fig:indirectMoran}d for CCA1, Fig. \ref{fig:TheoryCaseA}k for CaseC1), so that  
...
(Fig. \ref{fig:indirectMoran}e for CCA1, Fig. \ref{fig:TheoryCaseA}k for CaseC1). 
Interactions between Moran effects on long-timescales ($>8$y) were also similar for both CCA1 and
CaseC1, and for the same reason in both cases: on long timescales, sub-annual lags
...
%***DAN: Just rely on the regression results here, how the complex spectral stuff
(Fig. \ref{fig:indirectMoran}a v. p; see also Table 
...
immediate in both regions, though they were less pronounced in SB 
%***DAN: Same, not only for the negativity, but also for the fact that wave effects were less pronounced in SB.
(Fig. \ref{fig:indirectMoran}b,g,l v. q,v,aa; see also 
Table \ref{SM-tab:regcoefs}). 
%***DAN: The following setence will stay in main text, so we will have to keep a version of Fig. 8d and s,
%although most of the rest of that figure will be simplified away.
...
on both occassions \citep{Sheppard2019,CastoraniSheppardKelp}
; for this and other reasons (see below) interactions may also be common. 
...
the North Pacific Gyre Oscillation or El Ni\~{n}o Southern Oscillation (ENSO).
...
variables, and could then use our theory
to assess the potential for interacting Moran effects under a variety of biological scenarios. Since 
...
short, and they used methods more aligned with their goals of 
...
as in Figs \ref{fig:PedagogAlt}, \ref{fig:PedagogAlt_SM}, and as captured formally in our theory.
...
\cite{Sheppard2019} developed the wavelet methods applied by \cite{CastoraniSheppardKelp}.
...
Antagonistic interactions between Moran drivers may be one common and previously
...
This provides another argument, among many others, in support of 
...
%***DAN: If you end up adding any of these people to the author list, take them off here. Also in the acknowledgments for the README.
...
\caption{%\doublespacing 
Illustration of the main concept of synergistically (a, b) or antagonistically (c, d) interacting Moran effects.
Interactions require that each environmental variable itself be spatially synchronous; and then 
the alignment or misalignment of three types of lag (see below) determine the sign and strength of the interactions. 
...
an amount $l_{e2}$ for $\epsilon_i^{(2)}$, where these lags differ across the scenarios but are the same 
in all locations within a scenario.
Analogously, red arrows represent maximally negative effects.
Oscillations are separately spatially synchronous for each environmental variable in 
each scenario, illustrated with blue and pink rectangles: due to this environmental synchrony, peak positive population effects 
of the same variable occur at similar times across locations, and likewise for peak negative effects.
In the synergistic scenario, the lag between the environmental variables ($l_n$) and the lags of the population effects
of the variables ($l_{e1}$ and $l_{e2}$) are aligned, i.e., $l_n+l_{e1}=l_{e2}$, so that peak positive 
...
In the antagonistic scenario, lags are misaligned, i.e., $l_n+l_{e1}=l_{e2}+\sigma/2$, where $\sigma=20$ is the period, 
so that peak positive 
...
See the online version for color
renderings of all figures.}\label{fig:PedagogAlt}
\end{figure}
...
\caption{%\doublespacing 
Theoretical case studies A (panels a-d), B (panels e-h) and C (i-p, two alternative sets of 
...
Middle panels show the constituent parts of the theory (labeled). These constituents are complex-valued, 
...
Synch. contrib. = Synchrony contribution. See the online version for color
...
\caption{Similar to Fig. \ref{fig:PedagogAlt}, but for slightly altered 
scenarios. For Fig. \ref{fig:PedagogAlt}, for simplicity, we assumed that larger values of environmental variables had 
more positive influences on populations. We here assume that larger values of $\epsilon_i^{(1)}$ have more 
positive influences, but that smaller values of $\epsilon_i^{(2)}$ have more positive influences.
In scenario 1 here, $l_n+l_{e1}=l_{e2}$, but Moran interactions are nonetheless antagonistic because the 
altered direction of effects of $\epsilon_i^{(2)}$ means that the peak positive effects of that
variable correspond to the peak negative effects of $\epsilon_i^{(1)}$. In scenario 2 here, 
$l_n+l_{e1}=l_{e2}+\sigma/2$ for $\sigma=20$ the period, but Moran interactions are synergistic because the 
altered direction of effects of $\epsilon_i^{(2)}$ means that the peak positive effects of that
variable correspond to the peak positive effects of $\epsilon_i^{(1)}$. See the online version for color
renderings of all figures.}\label{fig:PedagogAlt_SM}
\end{figure}
...
\caption{%\doublespacing 
Main components of the new theory as applied to kelp, central CA 1 (CCA1) region (a-c) and 
...
\caption{%\doublespacing 
Components of direct Moran effects for kelp, for CCA1 (a-o)
...
\caption{%\doublespacing 
Components of interactions between Moran effects for kelp, for CCA1 (a-o) and SB (b-dd). 
...
See the online version for color
renderings of all figures.}\label{fig:indirectMoran}
\end{figure}
