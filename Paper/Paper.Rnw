...
%  -G1) Elaborate a mathematical theory of interacting Moran effects which allows a detailed understanding 
%      of how the mechanism works 
%  -G2) Consider theoretical case studies, which emerge as important special cases of our general theory, and which illuminate the intuition behind how Moran effects may interact in real systems. 
%  -G3) Apply the theory to populations giant kelp of the California coast, the intention being to further elaborate the mechanism itself
...
%4) Present "toy" examples and fig 1 (need to think a lot about these). DAN: perhaps these actually belong in Methods (describe the specific-case models we will consider) and results (apply the general theory to those specific cases)
%
%Notes:
...
oscillations, but peaks in the $\epsilon^{(1)}$ oscillations consistently lag peaks in the $\epsilon^{(2)}$ oscillations 
...
Linearization and ``weak noise'' assumptions are tactics which have been used very commonly to make
theoretical progress in ecology (REFS), and it has been demonstrated that results based on a
weak noise assumption often also hold for noise which is quite strong (REF).
%***DAN: Add refs later
...
the component of population synchrony due to the direct Moran effects of $\epsilon^{(2)}$, 
and the third term is the component due to interactions between the Moran effects of the 
two environmental drivers.
The ``other contributions'' in the above equations correspond to synchronizing influences of 
...
between Moran effects. The detailed derivation of the theory is in SI section \ref{SM-sect:derivation}.
...
facilitate later comparisons with results based on kelp data, which were available on a
...
$p_{n_1}^{(1)}>0$ while $p_i^{(1)}=0$ for $i \neq n_1$, and 
$p_{n_2}^{(2)}>0$ while $p_i^{(2)}=0$ for $i \neq n_2$.
The noise process
$(\epsilon^{(1)},\epsilon^{(2)})$ is assumed to be a Gaussian white-noise process 
for both A and B, i.e., 
...
each assumed to exhibit spatially synchronous periodic oscillations of period equal to one year, i.e.,
...
(Fig. \ref{fig:TheoryCaseA}a,e,i,m). First, for all case studies, the magnitude of 
interaction effects was comparable to that of direct Moran effects (compare the dashed and solid lines on 
Fig. \ref{fig:TheoryCaseA}a,e,i,m). So interactions can contribute substantially to overall population 
synchrony. Second, in contrast to the direct Moran effects of a single environmental variable, which provide non-negative 
contributions to population synchrony at all timescales for which environmental synchrony 
is positive, interactions between Moran effects can be positive or negative. For instance, case study A
showed negative interaction effects at short timescales (Fig. \ref{fig:TheoryCaseA}a); and, for some
parameters, case study C showed negative interactions at annual timescales (Fig. \ref{fig:TheoryCaseA}m).
...
straightforwardly. Additionally, those are case studies, so our results also provide novel theoretical generality;
and our theory can be interpreted to provide intuition about how interacting Moran effects work,
as follows. 
...
Moran effects on different timescales, and how our theory successfully captures that contrast.
...
2 quarters, producing destructive interference, i.e., negative interactions. Thus, on short timescales, total
...
This is because, on long timescales, a lag of 1 quarter does not meaningfully alter the phase relationship between 
...
at timescale 1 year (Fig. \ref{fig:TheoryCaseA}a). Theory effectively captures these complexities because
the size of the 1-quarter phase lag of this scenario, relative to a given timescale,
is captured by $f_{\mcP^{(1)}} \conj{f_{\mcP^{(2)}}}$, which has phase $-\pi$ at short timescales
and phase almost $0$ at long timescales (Fig. \ref{fig:TheoryCaseA}b). Because 
$\rho_{\epsilon^{(1)} \epsilon^{(2)}}$ has phase $0$ for all timescales, the phase of 
$f_{\mcP^{(1)}} \conj{f_{\mcP^{(2)}}} \rho_{\epsilon^{(1)} \epsilon^{(2)}}$ (Fig. \ref{fig:TheoryCaseA}c), which controls
the sign of the real part of this quantity, is the same as the phase of $f_{\mcP^{(1)}} \conj{f_{\mcP^{(2)}}}$.
Thus $2\Rp(f_{\mcP^{(1)}} \conj{f_{\mcP^{(2)}}} \rho_{\epsilon^{(1)} \epsilon^{(2)}})$, which is the
interaction term in our theory (\ref{rhoww_T1to3}), shifts 
...
quantity without altering the relative importance or sign of those effects [see (\ref{rhoww_T1to3})].

Case study B reveals how Moran effects can interact when lags are longer than 1 model time step. 
For B, $\epsilon^{(1)}$ effects on populations were lagged by 3 quarters but 
$\epsilon^{(2)}$ effects were unlagged (Methods). Thus interactions between Moran effects were 
negative on the shortest timescales 
(0.5 years = 2 quarters; Fig. \ref{fig:TheoryCaseA}e, left side of panel). This is because the 
population influences of Moran effects of timescale 2
...
Interaction effects on long timescales were instead synergistic, i.e., positive, for case study B 
(Fig. \ref{fig:TheoryCaseA}e, right side of panel), for the same reason as case study A (see above).
At timescale 3 quarters, Moran effects are, of course, reinforcing because the lag equals the timescale,
...
the size of the 3-quarter phase lag of this scenario, relative to a given timescale,
is captured by $f_{\mcP^{(1)}} \conj{f_{\mcP^{(2)}}}$ (Fig. \ref{fig:TheoryCaseA}f).
Because $\rho_{\epsilon^{(1)} \epsilon^{(2)}}$ is a real number for case study B,
the phase of $f_{\mcP^{(1)}} \conj{f_{\mcP^{(2)}}} \rho_{\epsilon^{(1)} \epsilon^{(2)}}$
is the same as that of $f_{\mcP^{(1)}} \conj{f_{\mcP^{(2)}}}$ (Fig. \ref{fig:TheoryCaseA}f); 
that common phase then controls the sign of 
$2\Rp(f_{\mcP^{(1)}} \conj{f_{\mcP^{(2)}}} \rho_{\epsilon^{(1)} \epsilon^{(2)}})$, which is the 
interaction effect term in (\ref{rhoww_T1to3}).

Cases A and B assumed white noise processes and real-valued $\rho_{\epsilon^{(1)} \epsilon^{(2)}}$,
but case study C illustrates what can happen when noise processes have periodic components, e.g., 
both noise processes oscillate with annual periodicity according to the seasons, an important scenario, ecologically. 
...
$\epsilon^{(1)}$ effects on populations were lagged (i.e., delayed in their influence) by 1 quarter, but 
$\epsilon^{(2)}$ effects were immediate (Methods and Fig. \ref{fig:TheoryCaseA} caption).
For scenario 1 of case study C (Fig. \ref{fig:TheoryCaseA}i-l), peaks in the periodic
noise process $\epsilon^{(2)}$ lagged peaks in the periodic process $\epsilon^{(1)}$ by
1 quarter. Thus lags cancelled and the annual peak population effects of $\epsilon^{(1)}$ and
$\epsilon^{(2)}$ arrived in sync with each other each year, producing positive interactions 
between Moran effects on annual timescales (Fig. \ref{fig:TheoryCaseA}i), and more population
synchrony than would have been expected if Moran effects operated independently. Contrastingly,
...
1 quarter. Thus lags compounded and the annual peak population effects of $\epsilon^{(1)}$
arrived 2 quarters after those of $\epsilon^{(2)}$ every year, a half-phase shift for an 
annually oscillating effect, which corresponds to negative interactions between Moran influences
and less population synchrony than would have been expected if Moran effects operated independently 
(Fig. \ref{fig:TheoryCaseA}m). The only difference between scenarios 1 and 2 of case study C
is the relative timing of annual peaks in $\epsilon^{(1)}$ and $\epsilon^{(2)}$.
Our theory captures these complexities because $f_{\mcP^{(1)}} \conj{f_{\mcP^{(2)}}}$
quantifies the relative phase lag between the effects of $\epsilon^{(1)}$ and 
$\epsilon^{(2)}$ relative to a given timescale, while 
$\rho_{\epsilon^{(1)} \epsilon^{(2)}}$ captures the phase lag between 
oscillations in the variables themselves, relative to the timescale.
The phase of $f_{\mcP^{(1)}} \conj{f_{\mcP^{(2)}}} \rho_{\epsilon^{(1)} \epsilon^{(2)}}$
...
reflected in the real part of this quantity.


%Current (2021 11 01) thoughts on figures:
%1) Maybe a pedagog fig, not sure what this would look like
%2) Combined theory fig, all 3 case studies
%3) "main fig" for CC1 and SB systems
%4-5) "explain direct effects" fig for CC1 and SB systems (2 figures, I think it'll have to be)
%6-7) "explain indirect effects" figs for CC1 and SB systems (again, 2)
%One too many figs here. What should be cut? Right now I don't know what the pedagog fig would be 
%so that's the obvious one to cut. And then it will all fit, though the figs are really big and 
%it seems like it is always good to have a pedagogical, warmup fig. If you do come up with a pedagog
%fig idea, could maybe combine some figs to make it work, or put the pedagog content as the first
%part of the theory fig? Don't worry about it for now, but return to it later.

\begin{figure}
\includegraphics[width=.75\textwidth]{../Results/Theory/TheoryFigCaseA.pdf}\\
\includegraphics[width=.75\textwidth]{../Results/Theory/TheoryFigCaseB.pdf}\\
\includegraphics[width=.75\textwidth]{../Results/Theory/TheoryFigCaseC_v01.pdf}\\
\includegraphics[width=.75\textwidth]{../Results/Theory/TheoryFigCaseC_v02.pdf}
...
%***DAN: More text here as we flesh out the presentation - need to describe the middle two columns of panels 
...
synchrony [see (\ref{rhoww_T1to3})-(\ref{rhoww_OC})].
Model parameters were: $N=5$ and $b_1=0.4$ for all case studies; 
$p_1^{(1)}=1.1$ and $p_0^{(2)}=0.8$ for case study A; 
$p_3^{(1)}=1.1$ and $p_0^{(2)}=0.8$ for case study B;
$p_1^{(1)}=3$, $p_0^{(2)}=1.5$ 
%$c_1=0$ and $c_2=-4/9$ 
...
and see Methods and SI section \ref{SM-sect:details_case_studies} for additional details.}\label{fig:TheoryCaseA}
\end{figure}
