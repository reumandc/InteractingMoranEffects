...
spatial synchronous. This alignment of produces additional synchrony in populations, 
...
to illuminate the inner workings of the general mechanism we describe. In the Discussion, we 
...
%***DAN: You might even consider a new panel or two somehow showing how the theory captures these effects more directly than 
%Fig. 3 currently shows, though not sure exactly how that would be done or if it's actually a good idea.
...
%***DAN: This para is a lot to track. It would probably help to have the new figures described above.
