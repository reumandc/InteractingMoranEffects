...
Spatial autocorrelations in fluctuating environmental drivers of populations are now known to be one of the most important
causes of synchrony, through a process called the Moran effect. But this effect was originally proposed 
theoretically \citep{Moran1953}, and decades passed during which it was 
considered difficult to distinguish this cause of synchrony from others, except under special circumstances.
Synchrony has long been thought to have three causes: dispersal, and trophic interactions with a synchronous or
mobile species, in addition to the Moran effect \citep{Liebhold2004}. But using common past approaches
examining declines in population correlations with distance, statistical patterns of synchrony produced
by each of these mechanisms are quite similar \citep{Ranta1999,Abbott2007,Liebhold2004,Walter2017}. So examination
of such patterns provides little or no traction for performing inferences about causes of synchrony. 
Early papers demonstrating the importance of Moran effects resorted to special cases where dispersal was 
impossible and predators were absent \citep{Grenfell1998,Tedesco2004}. Though controlled experiments revealed that all
three mechanisms could be involved in synchrony [e.g., \citealp{Vasseur2009}], the problem of determining
the relative contribution of each mechanism to synchrony observed in field systems was considered a challenge 
for decades.
...
Two recent papers have documented that Moran effects of distinct 
...
be more synchronous than expected. Although we considered only two drivers in 
our theory and examples, populations may, in principle, depend on many environmental
factors, all related to a large-scale climatic phenomenon such as 
ENSO and hence related to each other. Future work should consider whether and when such scenarios can lead to elevated synchrony. 
...
% %***DAN: Could def put this in sup mat if we're cutting, or drop it because it's a bit lame
