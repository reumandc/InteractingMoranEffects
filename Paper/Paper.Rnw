...
The components of the interacting Moran effects term are also interpretable.
...
corresponds to the phase of the expression 
...
$l_{e1}-l_{e2}+l_n$ is close to zero (e.g., Fig. \ref{fig:PedagogAlt}a, b), and is increasingly negative as 
...
(Fig. \ref{fig:PedagogAlt}c, d). The factor $1/|f_{\mcB}|^2$ is again how the intrinsic nature of population
dynamics modulates Moran influences.
The derivation of the theory and further information supporting the above interpretations 
are in SI section \ref{SM-sect:derivation}. 
...
language of Fig. \ref{fig:PedagogAlt}; whereas 
$\epsilon^{(2)}$ effects were unlagged and negative, i.e., $l_{e2}=0$ (Methods). 
We compare CaseC to the Fig. \ref{fig:PedagogAlt} $\beta$ scenarios instead of the Fig. \ref{fig:PedagogAlt} 
$\alpha$ scenarios because
...
Fig. \ref{fig:PedagogAlt}g, h: the 
...
similar to Fig. \ref{fig:PedagogAlt}e, f: the annual positive population effects of $\epsilon^{(1)}$
...
same in the two sub-cases, and were negative, because, on those timescales, sub-annual lags 
...
%audience.
Direct Moran effects for kelp and how they manifest in our theopry are fairly straightforward. 
...
synchrony had a strong annual component (Fig. \ref{fig:noise}a,b,j,k), which produced 
...
(Fig. \ref{fig:noise}g,h,p,q), possibly due to the North Pacific Gyre Oscillation 
...
had a stronger influence on kelp in CCA1 than in SB:
...
as in Fig. \ref{fig:PedagogAlt}, and as captured formally in our theory.
...
($\epsilon_i^{(2)}$ for $i=1,2,3$). For the scenarios $\alpha 1$ (a-b) and $\alpha 2$ (c-d), 
black arrows represent}\label{fig:PedagogAlt}
\end{figure}

\clearpage

\noindent peak positive influences of environmental 
...
$l_n+l_{e1}=l_{e2}+\sigma/2$, where $\sigma=20$ is the period, so that peak positive 
...
For the scenarios $\beta 1$ (e-f) and $\beta 2$ (g-h), we assume that larger values of $\epsilon_i^{(1)}$ have more 
positive population influences (as in the $\alpha$ scenarios), but that larger values of 
$\epsilon_i^{(2)}$ have more negative population influences (contrasting
with the $\alpha$ scenarios).
In scenario $\beta 1$, $l_n+l_{e1}=l_{e2}$, as in $\alpha 1$; but Moran interactions are nonetheless antagonistic because the 
...
See the online version for color renderings of all figures.
