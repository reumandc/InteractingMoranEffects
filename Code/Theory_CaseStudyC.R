...
#***
#The main function for doing both case studies, but this is a truncated version of the above that
#only plots some of the panels, for the main text of the paper.
#***
...
#Very little error checking, so watch out!
#
TheoryCaseC_MT<-function(b,p1,p2,N,ns,PlotName,PanLabs,PanLabs2,SyncYAx=NA,RelSyncYAx=NA,XAxOn=TRUE)
{
  #construct Sig, a covariance matrix used in construction of the noise, basically 
  #just to find out if it is positive definite
  w11<-ns[5]
  ...
  if (XAxOn)
  {
    par(fig=c((2*yaxwd+1*panwd)/totwd,
              (2*yaxwd+2*panwd)/totwd,
              (xaxht)/totht,
              (xaxht+panht)/totht),
        mai=c(0,0,0,0),mgp=c(3,.15,0),tcl=-.25,new=TRUE)
...
#***
#The main code, uses the above functions
#***
...
PlotName_MT<-paste0(resloc,"TheoryFigCaseC_v01_MT")
PanLabs<-c("(i)","(j)","(k)","(l)")
PanLabs_MT<-c("(e)","(f)")
PanLabs2<-c("(a)","(b)","(c)")
TheoryCaseC(b,p1,p2,N,ns,PlotName,PanLabs,PanLabs2,SyncYAx=c(-10,20),RelSyncYAx=c(-10,25),XAxOn=FALSE)
TheoryCaseC_MT(b,p1,p2,N,ns,PlotName_MT,PanLabs_MT,PanLabs2,SyncYAx=c(-10,20),RelSyncYAx=c(-10,25),XAxOn=FALSE)
...
PlotName_MT<-paste0(resloc,"TheoryFigCaseC_v02_MT")
PanLabs<-c("(m)","(n)","(o)","(p)")
PanLabs_MT<-c("(g)","(h)")
PanLabs2<-c("(d)","(e)","(f)")
TheoryCaseC(b,p1,p2,N,ns,PlotName,PanLabs,PanLabs2,SyncYAx=c(-10,20),RelSyncYAx=c(-10,25),XAxOn=TRUE)
TheoryCaseC_MT(b,p1,p2,N,ns,PlotName_MT,PanLabs_MT,PanLabs2,SyncYAx=c(-10,20),RelSyncYAx=c(-10,25),XAxOn=TRUE)
